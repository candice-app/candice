...
## Module 6c — Questionnaire incognito (parcours 3) (2026-05-27)

### Concept
Troisième parcours d'alimentation du profil proche : le Pilote remplit lui-même les réponses à la place de son proche. Distinct du parcours self-fill (proche remplit son propre profil) et du parcours lien (proche reçoit un lien et remplit son profil).

### Nouveau champ : gender
Ajouté sur la table `contacts` : `gender TEXT CHECK(femme|homme|non_binaire|non_precise)`. Utilisé pour les accords grammaticaux dans l'interface du questionnaire incognito.

### Nouveaux champs : questionnaire_responses
- `attention_reception JSONB` — FaceResult calculé à partir de Q1 (7D attention engine). Utilisé comme fallback `procheReception` dans le moteur quand le proche n'a pas de compte.
- `incognito_signals JSONB` — snapshot brut de toutes les réponses du questionnaire incognito.

### Route
`/contacts/[id]/questionnaire` — server component + client `IncognitoFlow`

### 5 stages, 8 questions + open + practical
| Q | Stage | Colonne DB | Type |
|---|-------|-----------|------|
| Q1 | Ce qui touche {{prenom}} | love_language, appreciation_style, attention_reception | Ranked ≤3 → FaceResult |
| Q2 | Ce qui touche {{prenom}} | emotional_expression | Ranked ≤3 |
| Q3 | L'univers de {{prenom}} | hobbies | Open text |
| Q4 | L'univers de {{prenom}} | gift_preference | Ranked ≤3 |
| Q5 | L'univers de {{prenom}} | standing | Single |
| Q6 | Comment {{prenom}} fonctionne | stress_response | Single |
| Q7 | Comment {{prenom}} fonctionne | decision_making | Ranked ≤2 |
| Q8 | Comment {{prenom}} fonctionne | things_to_avoid | Open text |
| Open 1 | Questions ouvertes | additional_notes | Open text |
| Open 2 | Questions ouvertes | conversation_topics | Open text |
| Practical | Infos pratiques | important_dates | Open text |

### 3 save points
1. **Après breath1** (Q1+Q2) : upsert `love_language`, `appreciation_style`, `emotional_expression`, `attention_reception`
2. **Après breath2** (Q3+Q4+Q5) : upsert `hobbies`, `gift_preference`, `standing`
3. **Practical (closing)** : upsert `stress_response`, `decision_making`, `things_to_avoid`, `additional_notes`, `conversation_topics`, `important_dates`, `incognito_signals`

### Accord grammatical
Fonction `interp(tpl, name, gender)` : remplace `{{prenom}}`, `{{il/elle}}`, `{{lui/elle}}`, `{{le/la}}`, `(e)`, `(es)`, `(ère)`, `(ée)` selon le genre.

### Moteur de recommandation
`/api/recommendations/generate/route.ts` : si `proche_user_id` est null ET `classicProfile.attention_reception` est non-null, utilise `classicProfile.attention_reception` comme `procheReception`.

### Fichiers créés/modifiés
- **`supabase-migration-20-incognito-questionnaire.sql`** — gender sur contacts, attention_reception + incognito_signals sur questionnaire_responses
- **`src/types/index.ts`** — `gender?` sur Contact, `attention_reception?` + `incognito_signals?` sur QuestionnaireResponse
- **`src/app/api/contacts/create-incognito/route.ts`** — accepte et sauvegarde `gender`
- **`src/components/contacts/NewContactFlow.tsx`** — gender radio (4 options) à l'étape 0 de IncognitoForm
- **`src/components/questionnaire/QuestionnaireForm.tsx`** — gender radio à l'étape 0 ; `handleChooseIncognito` crée le contact et redirige vers `/contacts/[id]/questionnaire` (supprime l'ancien mode inline step 3-4)
