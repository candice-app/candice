...
import { buildFallbackNarrative } from '@/lib/profile/synthesis';
import type { ProfileSynthesisFacts, SynthesisNarrative } from '@/lib/profile/synthesis';

const anthropic = new Anthropic();

const SYSTEM_PROMPT = `Tu es Candice, une présence intelligente et subtile. Tu dois rédiger les blocs narratifs d'une fiche profil personnelle à partir de faits structurés.
...
function buildPrompt(facts: ProfileSynthesisFacts): string {
  const lines: string[] = [
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    // Merge AI output with deterministic labels
    const narrative: SynthesisNarrative = {
      ...
      // Deterministic labels — never from AI
    ...
    // Add deterministic labels to fallback
