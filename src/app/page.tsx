...
// Dark constants for phone mockups (authentic app look)
const BR = "#1E1208";
const BR1 = "#261610";
const BR2 = "#301B12";
const BR3 = "#3A2015";
const CREAM = "#EFE7DC";
const CREAMD = "rgba(239,231,220,0.48)";
const BRD = "rgba(239,231,220,0.07)";
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14 }}>
      ...
      <MiniPhone title="Mes proches" label="Liste de proches">
        {[["SM", "Sophie Martin", "Paris · Amie", "u", "Anniversaire J-4"], ["TL", "Thomas Leroy", "Lyon · Frère", "n", "2 mois sans contact"], ["PL", "Paul Lemaire", "Paris · Ami", "g", "Marathon dimanche"], ["CP", "Claire — Maman", "Bordeaux", "u", "Fête des mères J-8"], ["JR", "Julien R.", "Nantes · Ami", "n", "3 mois sans contact"]].map(([init, name, meta, type, badge], i) => (
          <div key={i} style={{ display: "flex", alignItems: "center", gap: 7, padding: "7px 8px", background: BR, border: `0.5px solid ${BR3}`, borderRadius: 5, marginBottom: 4 }}>
            ...
            <div style={{ flex: 1 }}><div style={{ fontSize: 9, fontWeight: 400, color: CREAM }}>{name}</div><div style={{ fontSize: 7, fontWeight: 300, color: CREAMD }}>{meta}</div></div>
            <span style={{ fontSize: 6, letterSpacing: .5, textTransform: "uppercase", padding: "2px 5px", borderRadius: 2, background: type === "u" ? "rgba(196,122,74,0.16)" : type === "g" ? "rgba(74,124,89,0.14)" : "rgba(239,231,220,0.05)", color: type === "u" ? "#C47A4A" : type === "g" ? "#4A7C59" : CREAMD, border: `0.5px solid ${type === "u" ? "rgba(196,122,74,0.32)" : type === "g" ? "rgba(74,124,89,0.30)" : "transparent"}`, whiteSpace: "nowrap" }}>{badge}</span>
          </div>
        ))}
      </MiniPhone>
      {/* Phone 2 — Suggestions */}
      <MiniPhone title="Suggestions" label="Recommandations">
        {[{ who: "Sophie Martin", txt: "Bouquet de pivoines de chez Debeaulieu — livraison le 14 mai.", cta: "Commander", hi: true }, { who: "Paul Lemaire", txt: "Message d'encouragement pour son premier marathon. Prêt à envoyer.", cta: "Envoyer", hi: false }, { who: "Thomas Leroy", txt: "Déjeuner dimanche — créneau commun trouvé.", cta: "Confirmer", hi: false }].map((s, i) => (
          <div key={i} style={{ background: BR, border: `0.5px solid ${s.hi ? "rgba(196,122,74,0.32)" : BR3}`, borderRadius: 5, padding: 9, marginBottom: 5 }}>
            <p style={{ fontSize: 7, letterSpacing: 1.5, textTransform: "uppercase", color: "#C47A4A", marginBottom: 3 }}>{s.who}</p>
            <p style={{ fontSize: 8, fontWeight: 300, color: CREAMD, lineHeight: 1.45, marginBottom: 6 }}>{s.txt}</p>
            <button style={{ background: "#C47A4A", color: "#fff", fontSize: 7, padding: "3px 0", borderRadius: 2, border: "none", width: "100%", cursor: "pointer" }}>{s.cta}</button>
          </div>
        ))}
      </MiniPhone>
      {/* Phone 3 — Souvenirs */}
      <MiniPhone title="Souvenirs" label="Historique & souvenirs">
        {[{ d: "14", m: "Fév", who: "Sophie Martin", txt: "Bouquet de roses envoyé. Elle a adoré.", tag: "Réaction positive" }, { d: "02", m: "Jan", who: "Thomas Leroy", txt: "Message nouvel an. Lu et répondu.", tag: "Message · Lu" }, { d: "25", m: "Nov", who: "Claire", txt: "Dîner anniversaire. Excellent souvenir.", tag: "Expérience · Top" }, { d: "10", m: "Oct", who: "Paul Lemaire", txt: "Whisky Nikka lors de sa rupture.", tag: "Soutien · Impactant" }].map((mem, i) => (
          <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 7, background: BR, border: `0.5px solid ${BR3}`, borderRadius: 5, padding: 8, marginBottom: 4 }}>
            <div style={{ background: BR2, border: `0.5px solid ${BR3}`, borderRadius: 3, padding: "3px 5px", textAlign: "center", flexShrink: 0 }}>
              <div style={{ fontSize: 10, fontWeight: 500, color: CREAM, lineHeight: 1 }}>{mem.d}</div>
              <div style={{ fontSize: 6, color: CREAMD, textTransform: "uppercase", letterSpacing: 1 }}>{mem.m}</div>
            </div>
            <div>
              <p style={{ fontSize: 7, fontWeight: 500, color: "#C47A4A", marginBottom: 2 }}>{mem.who}</p>
              <p style={{ fontSize: 8, fontWeight: 300, color: CREAMD, lineHeight: 1.4 }}>{mem.txt}</p>
              <span style={{ display: "inline-block", marginTop: 3, fontSize: 6, letterSpacing: 1, textTransform: "uppercase", padding: "1px 5px", borderRadius: 2, background: "rgba(196,122,74,0.08)", color: "#C47A4A" }}>{mem.tag}</span>
            </div>
          </div>
        ))}
      </MiniPhone>
      ...
      <MiniPhone title="Sophie Martin" label="Fiche · KPIs · Matching">
        ...
        <p style={{ textAlign: "center", fontSize: 9, fontWeight: 500, color: CREAM, marginBottom: 1 }}>Sophie Martin</p>
        <p style={{ textAlign: "center", fontSize: 7, fontWeight: 300, color: CREAMD, marginBottom: 8 }}>Amie proche · Paris</p>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 4, marginBottom: 8 }}>
          {[["12", "Gestes"], ["94%", "Satisfaction"], ["J-4", "Anniversaire"], ["8", "Infos"]].map(([v, l]) => (
            <div key={l} style={{ background: BR, border: `0.5px solid ${BR3}`, borderRadius: 4, padding: 5, textAlign: "center" }}>
              <div style={{ fontSize: 14, fontWeight: 300, color: "#C47A4A", letterSpacing: -.5, lineHeight: 1 }}>{v}</div>
              <div style={{ fontSize: 6, fontWeight: 300, color: CREAMD, marginTop: 2, textTransform: "uppercase", letterSpacing: 1 }}>{l}</div>
            </div>
          ))}
        </div>
        <div style={{ background: BR, border: `0.5px solid ${BR3}`, borderRadius: 4, padding: 7 }}>
          <p style={{ fontSize: 6, letterSpacing: 1.5, textTransform: "uppercase", color: "#C47A4A", marginBottom: 6 }}>Profil de réception</p>
          {[["Cadeau", 85], ["Expé.", 72], ["Mots", 60], ["Geste", 45]].map(([l, v]) => (
            <div key={l as string} style={{ display: "flex", alignItems: "center", gap: 4, marginBottom: 3 }}>
              <span style={{ fontSize: 6, color: CREAMD, width: 28, textAlign: "right", flexShrink: 0 }}>{l}</span>
              <div style={{ flex: 1, height: 3, background: BR3, borderRadius: 2, overflow: "hidden" }}>
                <div style={{ width: `${v}%`, height: "100%", background: "#C47A4A", borderRadius: 2 }} />
              </div>
              <span style={{ fontSize: 6, color: CREAMD, width: 16, textAlign: "right", flexShrink: 0 }}>{v}%</span>
            </div>
          ))}
        </div>
      </MiniPhone>
...
function MiniPhone({ title, label, children }: { title: string; label: string; children: React.ReactNode }) {
  return (
    <div style={{ background: BR2, border: `0.5px solid ${BR3}`, borderRadius: 18, padding: 8 }}>
      <div style={{ width: 32, height: 3, background: BR3, borderRadius: 2, margin: "0 auto 7px" }} />
      <div style={{ background: BR1, borderRadius: 12, padding: 9, overflow: "hidden" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8, paddingBottom: 7, borderBottom: `0.5px solid ${BRD}` }}>
          <span style={{ fontSize: 8, fontWeight: 500, letterSpacing: 1, textTransform: "uppercase", color: CREAM }}>{title}</span>
          <span style={{ fontSize: 7, color: "#C47A4A" }}>+ Ajouter</span>
        </div>
        {children}
      </div>
