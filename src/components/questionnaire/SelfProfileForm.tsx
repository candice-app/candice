  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
        ...
        {values.length > 0 && (
          <span style={{ fontSize: 10, fontWeight: 500, color: "var(--terra)", marginLeft: 12, flexShrink: 0, background: "var(--t2)", padding: "2px 8px", borderRadius: 10, border: "0.5px solid var(--t3)" }}>
            {values.length}/{max}
          </span>
        )}
      </div>
      <div className="pill-grid">
        {options.map((opt) => {
          ...
          return (
            <button key={opt.value} type="button" onClick={() => !disabled && toggle(opt.value)}
              ...
              {opt.description && (
                <span style={{ fontSize: 13, fontWeight: 300, lineHeight: 1.35, color: selected ? "rgba(255,255,255,0.8)" : "rgba(30,18,8,0.6)", paddingLeft: selected ? 24 : 0, overflowWrap: "break-word", wordBreak: "break-word" }}>
                  {opt.description}
                </span>
              )}
              {selected && (
...
function parseImportantDates(raw: string | null): ImportantDate[] {
  ...
  } catch { /* legacy free text — start fresh */ }
...
export default function SelfProfileForm({ userId, initial }: Props) {
  const router = useRouter();
  const supabase = createClient();
  const DRAFT_KEY = `candice_my_profile_draft_${userId}`;
  ...
  const va = (key: keyof MyProfile): string[] => {
    const val = initial?.[key] as string | null;
    if (!val) return [];
  ...
  const [appreciationStyle, setAppreciationStyle] = useState<string[]>(va("appreciation_style"));
  const [conflictResolution, setConflictResolution] = useState<string[]>(va("conflict_resolution"));
  const [decisionMaking, setDecisionMaking] = useState<string[]>(va("decision_making"));
  const [emotionalExpression, setEmotionalExpression] = useState<string[]>(va("emotional_expression"));
  const [coreValues, setCoreValues] = useState<string[]>(va("core_values"));
  const [recognitionPreference, setRecognitionPreference] = useState<string[]>(va("recognition_preference"));
  const [boundaries, setBoundaries] = useState<string[]>(va("boundaries"));
  const [growthMindset, setGrowthMindset] = useState<string[]>(va("growth_mindset"));
  // Preferences
  const [hobbies, setHobbies] = useState(v("hobbies"));
  const [dislikedActivities, setDislikedActivities] = useState(v("disliked_activities"));
  const [favoriteFoods, setFavoriteFoods] = useState(v("favorite_foods"));
  const [dislikedFoods, setDislikedFoods] = useState(v("disliked_foods"));
  const [giftPreference, setGiftPreference] = useState<string[]>(va("gift_preference"));
  const [standing, setStanding] = useState<string[]>(va("standing"));
  const [gastronomy, setGastronomy] = useState<string[]>(va("gastronomy"));
  const [accommodation, setAccommodation] = useState<string[]>(va("accommodation"));
  const [giftStyle, setGiftStyle] = useState<string[]>(va("gift_style"));
  const [tactility, setTactility] = useState<string[]>(va("tactility"));
  const [conversationTopics, setConversationTopics] = useState(v("conversation_topics"));
  const [thingsToAvoid, setThingsToAvoid] = useState(v("things_to_avoid"));
  const [bestContactMethod, setBestContactMethod] = useState<string[]>(va("best_contact_method"));
  ...
  const [characterEmotions, setCharacterEmotions] = useState(v("character_emotions"));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err) {
        setError("Impossible d'enregistrer ta fiche. Tes réponses sont conservées — réessaie dans un instant.");
        return;
      }
      ...
      setSaved(true);
      await awardPoints(userId, initial ? "profile_update" : "profile_complete");
      router.push("/moi");
      router.refresh();
    } catch {
      setError("Une erreur inattendue s'est produite. Tes réponses sont conservées — réessaie dans un instant.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 0 }}>
      ...
      {error && (
        <p style={{ fontSize: 13, color: "#E05252", background: "rgba(224,82,82,0.08)", border: "0.5px solid rgba(224,82,82,0.25)", borderRadius: 6, padding: "10px 14px", marginBottom: 20 }}>
          {error}
        </p>
      )}
      {saved && (
        <p style={{ fontSize: 13, color: "var(--green)", background: "var(--green2)", border: "0.5px solid var(--green3)", borderRadius: 6, padding: "10px 14px", marginBottom: 20 }}>
          Fiche enregistrée.
        </p>
      )}
      ...
      <div className="card-light" style={{ display: "flex", flexDirection: "column", gap: 28, marginBottom: 32 }}>
        ...
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {importantDatesList.map((row, i) => (
              <div key={i} style={{ display: "flex", gap: 8, alignItems: "center" }}>
                {row.isCustom ? (
                  ...
                  <span style={{ flex: 1, fontSize: 13, fontWeight: 400, color: "#1E1208", padding: "10px 14px", background: "var(--iv2)", borderRadius: "var(--r-md)", border: "0.5px solid var(--iv3)", display: "block" }}>
                    {row.label}
                  </span>
                )}
                <input
                ...
                {row.isCustom && (
                  <button
                    type="button"
                    onClick={() => setImportantDatesList(importantDatesList.filter((_, j) => j !== i))}
                    style={{ background: "none", border: "none", color: "var(--txts)", fontSize: 18, cursor: "pointer", padding: "0 2px", flexShrink: 0, lineHeight: 1 }}
                  >×</button>
                )}
              </div>
            ))}
            <button
              type="button"
              onClick={() => setImportantDatesList([...importantDatesList, { label: "", date: "", isCustom: true }])}
              className="btn-ghost"
      ...
      <div className="card-light" style={{ display: "flex", flexDirection: "column", gap: 28, marginBottom: 32 }}>
        ...
        <div>
          ...
          <textarea value={religion} onChange={(e) => setReligion(e.target.value)} rows={2}
            placeholder="ex. catholique pratiquant, bouddhiste, agnostique, athée convaincu…" />
        </div>
        ...
        <div>
          ...
          <textarea value={disability} onChange={(e) => setDisability(e.target.value)} rows={2}
            placeholder="ex. malvoyant(e), fauteuil roulant, troubles auditifs, douleurs chroniques…" />
        </div>

        <div>
          <label style={FIELD_LABEL}>Adresse postale (pour les livraisons)</label>
          <textarea value={postalAddress} onChange={(e) => setPostalAddress(e.target.value)} rows={2}
            placeholder="ex. 12 rue de la Paix, 75001 Paris — ou adresse du bureau si tu préfères" />
        </div>
      </div>
      ...
      <div style={{ position: "fixed", bottom: 24, right: 24, zIndex: 100, display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 8 }}>
        ...
        <button
          ...
          style={{ background: "#fff", border: "1.5px solid #C47A4A", color: "#C47A4A", borderRadius: 8, padding: "10px 16px", fontSize: 14, fontWeight: 500, cursor: "pointer", boxShadow: "0 2px 12px rgba(0,0,0,0.12)" }}
        >
