...
export default function BottomNav() {
  ...
  return (
    ...
    >
      {/* Filet */}
      <div style={{
        position: 'absolute', left: 26, right: 26, top: 8,
      ...
      {NAV_ITEMS.map((item, i) => {
        if (item.center) {
          return (
            <Link
              key="center"
              href={item.href}
              aria-label="Parler à Candice"
              style={{
                flex: '0 0 auto', margin: '0 4px',
                transform: 'translateY(-11px)', textDecoration: 'none',
              }}
            >
              <div style={{
        ...
        const active = (item as { activeOn?: (p: string) => boolean }).activeOn?.(pathname) ?? false;
        return (
          <Link
            key={item.href + i}
            href={item.href}
            style={{
              ...
              textDecoration: 'none', flex: 1, transition: 'color .3s',
            }}
          >
            {item.icon}
            <span style={{ fontSize: 9.5, letterSpacing: '.06em', fontWeight: active ? 500 : 300 }}>
              {item.label}
            </span>
            {active && (
              <span style={{
                position: 'absolute', top: 14,
                width: 3, height: 3, borderRadius: '50%', background: 'var(--champ)',
              }} />
            )}
          </Link>
        );
